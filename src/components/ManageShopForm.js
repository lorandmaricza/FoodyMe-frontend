...
export default function ManageShopForm(props) {
         ...
         const fetchShops = async () => {
             try {
                const response = await fetch(
                    'http://localhost:8888/final-project-back-end/public/shop/get-shop.php',
                    {
                        method: "POST",
                        mode: "cors",
                        credentials: "include",
    ...
    const handleDelete = async () => {
        try {
            const response = await fetch('http://localhost:8888/final-project-back-end/public/shop/delete-shop.php', {
                method: 'POST',
                body: JSON.stringify({ shopId }),
            });
            await response.json();
